...
export const Services = class {
  ...
  _onMountAdded(monitor, mount) {
    ...
    // let appname = `mount-${this._toSafeFileName(basename)}-liquid-dash-animated.desktop`;
  ...
  setupTrashIcon() {
    ...
    let appname = `trash-liquid-dash-animated.desktop`;
    ...
    let content = `[Desktop Entry]\nVersion=1.0\nTerminal=false\nType=Application\nName=Trash\nExec=${open_app} trash:///\nIcon=user-trash\nStartupWMClass=trash-liquid-dash-animated\nActions=trash\n\n[Desktop Action trash]\nName=Empty Trash\nExec=${trash_action}\nTerminal=true\n`;
  ...
  setupFolderIcon(name, title, icon, path) {
    ...
    let extension_path = this.extension.path;
    let appname = `${name}-liquid-dash-animated.desktop`;
    let app_id = tempPath(appname);
    let fn = Gio.File.new_for_path(app_id);
    // let open_app = 'xdg-open';
    let open_app = 'nautilus --select';

    let content = `[Desktop Entry]\nVersion=1.0\nTerminal=false\nType=Application\nName=${title}\nExec=${open_app} ${full_path}\nIcon=${icon}\nStartupWMClass=${name}-liquid-dash-animated\n`;
    const [, etag] = fn.replace_contents(
      content,
      null,
      false,
