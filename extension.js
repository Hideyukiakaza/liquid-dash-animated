...
export default class LiquidDashAnimatedExt extends Extension {
  createDock() {
  ...
  enable() {
    ...
    console.log('Liquid Dash Animated by Hideyukiakaza enabled');
  ...
  disable() {
    ...
    Main.overview.ldal = null;
    console.log('Liquid Dash Animated by Hideyukiakaza disabled');
  }
  ...
  async _loadConfig() {
    this._config = {};
    let fn_config = Gio.File.new_for_path('.config/lda/config.json');
    ...
    let fn_icons = Gio.File.new_for_path('.config/lda/icons.json');
        ...
        if (json['icons']) {
          ...
          Object.keys(this.icon_map).forEach((k) => {
            ...
            if (path.toLowerCase().endsWith('.svg')) {
              let file = Gio.File.new_for_path(`.config/lda/${path}`);
    ...
    let fn_style = Gio.File.new_for_path('.config/lda/style.css');
  ...
  _updateStyle(disable) {
    ...
    if (this.icon_shadow) {
      styles.push(
        '#ldaIcon, #DockItemList StIcon {icon-shadow: rgba(0, 0, 0, 0.32) 0 2px 6px;}'
      );
      // styles.push(
    ...
    // dash
    {
      ...
      styles.push(`#ldaBackground { ${ss.join(' ')}}`);
    }
    ...
    this._style.build('custom-ldal', styles);
